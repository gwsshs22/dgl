import argparse
import time

from common import run_exp, LatencyExpParams

def turn_off_memory_cache(graph_name, gnn, num_layers, recom_threshold):
    if graph_name == "yelp":
        return False
    if graph_name == "amazon" and recom_threshold <= 45:
        return True

    return False

def main(args):
    print(f"Start run_precoms.py args={args}", flush=True)
    start_t = time.time()
    gnn_names = ["gcn", "gat"]
    graph_names = ["yelp", "amazon"]
    exec_types = ["cgp-multi", "cgp", "dp-precoms"]

    extra_env_names = []
    if args.extra_env_names:
        extra_env_names = args.extra_env_names.split(",")
    batch_size = 1024

    exp_result_dir = f"{args.exp_root_dir}/precoms"
    recom_thresholds = [100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 
        87, 86, 85, 84, 83, 82, 81, 80, 79, 78, 77, 76, 75, 74, 73, 72, 71, 70, 
        65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 5, 0]
    for graph_name in graph_names:
        for gnn in gnn_names:
            for recom_threshold in recom_thresholds:
                for exec_type in exec_types:
                    num_reqs = args.num_reqs
                    num_layers = 2 if gnn == "gcn" else 3

                    run_exp(
                        num_machines=4,
                        graph_name=graph_name,
                        gnn=gnn,
                        num_layers=num_layers,
                        fanouts=[],
                        exec_type=exec_type,
                        exp_type="latency",
                        latency_exp_params=LatencyExpParams(num_reqs=num_reqs),
                        batch_size=batch_size,
                        exp_result_dir=f"{exp_result_dir}/{graph_name}_{gnn}_{exec_type}_rt_{recom_threshold}",
                        extra_env_names=extra_env_names,
                        recom_threshold=recom_threshold,
                        force_cuda_mem_uncached=turn_off_memory_cache(graph_name, gnn, num_layers, recom_threshold)
                    )

    print(f"Total experiments time={time.time() - start_t}s", flush=True)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--exp_root_dir', required=True)
    parser.add_argument('--num_reqs', type=int, default=500)
    parser.add_argument('--extra_env_names', type=str, default="")

    args = parser.parse_args()
    main(args)
