...
def main(args):
    print(f"Start run_features.py args={args}", flush=True)
    start_t = time.time()
    ...
    batch_size = 1024
    gnn = "sage"
    exp_result_dir = f"{args.exp_root_dir}/features"
        ...
        for feature_dim in feature_dims:
            for exec_type in exec_types:
                run_exp(
                    ...
                    exp_result_dir=f"{exp_result_dir}/{graph_name}_{feature_dim}_{exec_type}_sampled",
                ...
                run_exp(
                    ...
                    batch_size=batch_size,
                    feature_dim=feature_dim,
                    exp_result_dir=f"{exp_result_dir}/{graph_name}_{feature_dim}_{exec_type}_full",
                    extra_env_names=extra_env_names
                )
