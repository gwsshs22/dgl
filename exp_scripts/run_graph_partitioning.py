...
def main(args):
    print(f"Start run_graph_partitioning.py args={args}", flush=True)
    start_t = time.time()
    ...
    graph_names = ["yelp", "amazon", "ogbn-products"]
    exec_types = ["cgp-multi", "dp-sampled", "dp"]
    graph_partitioning_methods = ["random", "metis"]
    gnn = "sage"
    num_layers = 3
    exp_result_dir = f"{args.exp_root_dir}/graph_partitioning"
    batch_size = 1024

    for graph_name in graph_names:
        for exec_type in exec_types:
            for graph_partitioning_method in graph_partitioning_methods:
                ...
                run_exp(
                    ...
                    exp_result_dir=f"{exp_result_dir}/{graph_partitioning_method}_{graph_name}_{gnn}_{exec_type}",
                    extra_env_names=extra_env_names,
                    graph_partitioning=graph_partitioning_method,
