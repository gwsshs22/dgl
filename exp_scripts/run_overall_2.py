...
def turn_off_memory_cache(graph_name, gnn, num_layers):
...
def main(args):
    print(f"Start run_overall_2.py args={args}", flush=True)
    start_t = time.time()
        ...
        for gnn in gnn_names:
            for exec_type in exec_types:
                ...
                run_exp(
                    ...
                    recom_threshold=100,
                    force_cuda_mem_uncached=turn_off_memory_cache(graph_name, gnn, num_layers)
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_reqs', type=int, default=50)
