...
void MasterProcessMain(caf::actor_system& system, const config& cfg) {
  ...
  auto res_hdl = self->request(init_mon_actor, std::chrono::minutes(180), caf::wait_atom_v, required_actors);
  ...
  auto warmup_rh = self->request(result_receiver, std::chrono::minutes(360), caf::wait_warmup_atom_v);
